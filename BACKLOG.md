...
- [ ] Put client id/secret in `.keys` on macmini
...
### 3. WebOne production deploy (owner + admin@webone)

- [ ] Target: **gmcp.c52.com on WebOne** — NOT the macmini
...
- [ ] systemd + pyenv on WebOne (see memory `deploy/webone-shape`)
- [ ] admin@webone holding folder creation until remote + HTTP side verified
...
- Pointing Cloudflare/gateway at macmini (HTTP is loopback only here)
...
- `memory:deploy/webone-shape` — WebOne runtime shape
