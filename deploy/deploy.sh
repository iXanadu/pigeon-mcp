#!/usr/bin/env bash
# pigeon-mcp deploy for WebOne. Run by ANY operator with sudo — no admin agent
...
# git ops need ixanadu's GitHub key; file ops need the gmcp_user group.
...
verify(){   # returns 0 only if the service is genuinely serving
  ...
  # MUST be sudo: the tokens dir is 0750 gmcp_user, so an unprivileged find
