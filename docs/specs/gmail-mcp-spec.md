...
Use Google OAuth 2.0 with a refresh token (offline access). One **Web application** OAuth client the operator owns, redirect `https://<host>/oauth/callback`; consent starts from the agent (`accounts_auth_start`) and completes in any browser. A Desktop client is an optional stdio-only convenience. Each Gmail identity is a separate consent. Tokens live on the MCP host as files whose names contain `token` (mode 0640 so a group-readable backup can see them), never in chat, never in a tool argument.
