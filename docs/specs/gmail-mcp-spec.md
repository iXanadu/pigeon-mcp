...
Hand (and Inbox) have to talk to any Gmail account the operator adds: today Homedog, TAG, Personal; tomorrow a fourth without a code change.
...
After the live signature, add one line: `Sent by Hand, Robert's assistant.`
...
Adding Homedog, TAG, Personal, or a fourth is OAuth, not a deploy. One server, N tokens.
...
Run these against two real boxes the operator names (Homedog → Personal is enough).
...
- AgentMail. Different MTA. Not Robert's three boxes.
...
Hand says send on any connected Gmail. The counterpart gets Robert, from that box, on that thread, with the signature that is in Gmail right now. The file opens. The link goes where we wrote it. Sent mail matches. A new Gmail is one OAuth, not a rewrite.
