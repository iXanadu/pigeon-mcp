...
- PROD HOST IS WEBONE (`gmcp.c52.com`, Cloudflare-proxied), NOT the Mac —
  learned from admin@webone 2026-08-26 8:48 pm EDT. Deploy follows WebOne's
  ...
  `/var/www/gmcp.c52.com/prod`, Linux user `gmcp_user`. FastMCP's
  Streamable HTTP transport is an ASGI app, so it serves under uvicorn like
  every other WebOne site. Full record: gmail-mcp memory `deploy/webone-shape`.
  admin@webone is HOLDING provisioning until this repo has a remote and the
...
- NO OAuth callback route on the vhost: Desktop-client consent runs once per
  ...
  WebOne. Public surface = `/mcp` only.
- SECRETS ON WEBONE ARE 0640 app-user:group (ixanadu in the group), NOT the
  spec's 0600: FleetVault's vault-backup runs as ixanadu every 30 min and a
...
- WebOne has no blob backup. Keep no mail cache (spec agrees). The one local
...
- Edge: Cloudflare Browser Integrity Check 403s non-browser clients unless
  the host has its own Configuration Rule (engram and agentbeast have one;
