...
- Production HTTP is loopback ASGI behind a reverse proxy (not the
  operator's laptop). Public path is `/mcp` only. Desktop OAuth consent
  runs on a local machine (loopback redirect); token files are copied to
  the server. No OAuth callback on the public vhost.
- Token files on a shared backup host may need group-readable mode (0640)
  if a backup agent cannot read 0600. Prefer 0600 when backup can.
- Idempotency store is local state; restoring without it can duplicate a
  send. Do not cache mail.
- Put a Cloudflare (or equivalent) rule so non-browser MCP clients are not
  blocked by bot checks, plus an edge credential on top of the app bearer.
